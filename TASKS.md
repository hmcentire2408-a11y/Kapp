...
| # | Task | Status | Acceptance criteria |
|---|---|---|---|
| B1 | **Library** → `corpus/library/` | todo | Copy and scrub: |
...
| B2 | **Morganton** → `corpus/morganton/` | todo | Copy and scrub: |
...
| B3 | `scripts/scrub_check.py` + clean run | todo | Scans `corpus/` (and later `evals/`) case-insensitively for the denylist in `.denylist.local` plus the path patterns `applicant/`, `state/`, `planning/`, `STATUS.md`, `UPDATE-PROMPT`; prints file:line hits; exits non-zero on any hit. Final run: zero hits. |
| B4 | `corpus/MANIFEST.md` | todo | Per Interfaces above. |
